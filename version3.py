...
def detect_dust(img, cx, cy, r):
    ...
    # Binary canvas: keep only confirmed dust blobs (reject noise by area)
    dust_binary = np.zeros_like(white)
    ...
    for cnt in cnts:
        ...
        if DUST_MIN_AREA <= area <= DUST_MAX_AREA:
            ...
            # Paint confirmed blob white on the clean binary mask
            cv2.drawContours(dust_binary, [cnt], -1, 255, -1)

    # Return hits + binary crop + its top-left offset so caller can stitch
    return dust_hits, dust_binary, (x1, y1)
...
def main():
    ...
    original    = img.copy()
    output      = img.copy()
    # Full-image binary canvas — starts all black, dust pixels go white
    binary_full = np.zeros(img.shape[:2], dtype=np.uint8)
    ...
    for isl_num, island_box in enumerate(islands):
        ...
        for (ocx, ocy, or_) in outer_rings:
            ...
            dust, bin_crop, (bx0, by0) = detect_dust(img, icx, icy, ir)
            count = len(dust)
            total_dust += count
            print(f"          Dust particles: {count}")

            # Stitch this lens's binary crop onto the full-image canvas
            bh, bw = bin_crop.shape
            y2b = min(by0 + bh, binary_full.shape[0])
            x2b = min(bx0 + bw, binary_full.shape[1])
            binary_full[by0:y2b, bx0:x2b] = cv2.bitwise_or(
                binary_full[by0:y2b, bx0:x2b],
                bin_crop[:y2b - by0, :x2b - bx0]
            )
            # Draw inner lens boundary on binary image so ROI is visible
            cv2.circle(binary_full, (icx, icy), ir, 128, 1)
    ...
    cv2.imshow("Binary — Dust Mask",
               cv2.resize(binary_full, disp) if scale < 1 else binary_full)

    cv2.imwrite("dust_result.png", output)
    cv2.imwrite("dust_binary.png", binary_full)
    print(f"\n[Done] Results saved → dust_result.png  |  dust_binary.png")
